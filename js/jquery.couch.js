...
(function($) {
  ...
  $.extend($.couch, {
    urlPrefix: 'http://www.qolt.kr/qmap',
	 ...
	 */	
        find: function(name, options) {
          ...
          ajax({
              type: type,
              data: data,
              url: $.couch.urlPrefix + "/_fti/local/" + this.name  + "/_design/" + name + encodeOptions(options)
            },
            options, "An error occurred accessing the view"
          );
        },
  ...
  function ajax(obj, options, errorMessage, ajaxOptions) {
    ...
    $.ajax($.extend($.extend({
      type: "GET", dataType: "json", cache : navigator.appVersion.indexOf("MSIE") == -1,
      beforeSend: function(xhr){
